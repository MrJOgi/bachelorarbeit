...
Die im vorherigen Abschnitt dargelegten Kommunikationswerkzeuge der Allianz erfordern für eine einwandfreie Funktion den aktuellen Bestand der personenbezogenen
Daten der Angestellten, die durch drei zentrale Personalsysteme der Allianz bereitgestellt werden. Hierzu gehören die beiden \glsdisp{sap}{SAP
(Systemanalyse Programmentwicklung)}-Produkte \gls{hcm} und \gls{sf} sowie \gls{giam}. Diese bilden die technische Grundlage firmeninterner Systeme und
werden im Rahmen dieses Abschnitts behandelt.

\subsubsection{SAP Human Capital Management (HCM)}
Allianz Personalsystem
Anpassung der Schnittstelle --> migration auf SF --> Gehaltsabrechnung nur
\subsubsection{Global Identity Access Management (GIAM)}
\subsubsection{SAP SuccessFactors (SF)}